...
let fixpoint : Fixpoint.t Fmt.t =
  fun ppf t -> match t with
    | Least    -> Fmt.string ppf "u"
    | Greatest -> Fmt.string ppf "v"
...
let rec divide_args_and_body (expr: 'ty Hflz.t) =
  match expr with
  | Abs(x, psi) -> 
    let (args, body) = divide_args_and_body psi in
    (x::args, body)
  | _ -> ([], expr)

let hflz_hes_rule : (Prec.t -> 'ty Fmt.t) -> 'ty Hflz.hes_rule Fmt.t =
  fun format_ty_ ppf rule ->
    let (args, body) = divide_args_and_body rule.body in
    Fmt.pf ppf "@[<2>%s %s =%a@ %a.@]"
      (Id.to_string rule.var)
      (ids args)
